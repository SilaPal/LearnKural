...
interface PageHeaderProps {
    ...
    /** If true, aligns the title next to the logo on the left instead of centering it. */
    alignLeft?: boolean;
        ...
        <div suppressHydrationWarning className="w-full">
            ...
            <header className={`${gradientClass} text-white shadow-lg py-5 sm:py-7 relative`}>
                <div className={`${maxWidthClass} mx-auto px-4 flex items-center justify-between`}>
                    {/* Left: back arrow + logo + (optional) left-aligned title */}
                    <div className="flex items-center gap-3 z-10">
                        ...
                        <Link href="/" className="flex items-center gap-3">
                            ...
                            {alignLeft && (
                                <span className="font-bold text-white text-2xl sm:text-3xl tracking-tight whitespace-nowrap drop-shadow-md hidden sm:block">
                                    {title || (isTamil ? 'திருக்குறள்' : 'Thirukkural')}
                                </span>
                            )}
                        </Link>
                    </div>

                    {/* Centered title (only if not alignLeft) */}
                    {!alignLeft && (
                        <Link
                            href="/"
                    ...
                    <div className="absolute top-0 bottom-0 right-4 flex items-center gap-2 z-50" ref={menuRef}>
                        {hideSubBar && (
                            <button
                                ...
                                className="flex items-center gap-2 px-3 py-1.5 bg-white/10 hover:bg-white/20 border border-white/20 text-white rounded-lg transition-colors text-xs font-semibold shadow-sm mr-2 backdrop-blur-sm"
                            >
