...
export default function HomeClient({ totalKurals, kuralOfDay, firstKuralSlug, allKuralSlugs }: HomeClientProps) {
  ...
  return (
    <>
      <header className="bg-gradient-to-br from-purple-800 via-purple-600 to-violet-500 text-white py-6 sm:py-8">
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 max-w-6xl">
        ...
        <div className="mb-6 bg-gradient-to-br from-purple-100 to-violet-100 rounded-lg shadow-md overflow-hidden border border-purple-200">
          <div className="p-6">
            ...
            {/* Game Mode - Link to games page */}
            {kuralMode === 'game' && (
              <div className="mt-3 bg-gradient-to-br from-orange-50 to-red-50 rounded-xl p-6 border-2 border-orange-200">
                <div className="text-center mb-4">
                  <div className="text-4xl mb-2">🎮</div>
                  <h3 className="text-lg font-bold text-orange-800">
                    {isTamil ? 'குறள் விளையாட்டுகள்' : 'Kural Games'}
                  </h3>
                  <p className="text-sm text-orange-700 mt-1">
                    {isTamil 
                      ? '4 வேடிக்கையான விளையாட்டுகள் மூலம் குறள் கற்கவும்!'
                      : 'Learn kurals through 4 fun interactive games!'}
                  </p>
                </div>
                
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <Link href={`/kural-playing?game=puzzle&kural=${kuralOfDay.id}`} className="bg-white rounded-lg p-3 text-center border border-blue-200 hover:shadow-md hover:scale-105 transition cursor-pointer">
                    <span className="text-2xl">🧩</span>
                    <p className="text-xs font-medium text-gray-700 mt-1">{isTamil ? 'புதிர்' : 'Puzzle'}</p>
                  </Link>
                  <Link href={`/kural-playing?game=flying&kural=${kuralOfDay.id}`} className="bg-white rounded-lg p-3 text-center border border-purple-200 hover:shadow-md hover:scale-105 transition cursor-pointer">
                    <span className="text-2xl">🦋</span>
                    <p className="text-xs font-medium text-gray-700 mt-1">{isTamil ? 'பறக்கும்' : 'Flying'}</p>
                  </Link>
                  <Link href={`/kural-playing?game=balloon&kural=${kuralOfDay.id}`} className="bg-white rounded-lg p-3 text-center border border-red-200 hover:shadow-md hover:scale-105 transition cursor-pointer">
                    <span className="text-2xl">🎈</span>
                    <p className="text-xs font-medium text-gray-700 mt-1">{isTamil ? 'பலூன்' : 'Balloon'}</p>
                  </Link>
                  <Link href={`/kural-playing?game=race&kural=${kuralOfDay.id}`} className="bg-white rounded-lg p-3 text-center border border-green-200 hover:shadow-md hover:scale-105 transition cursor-pointer">
                    <span className="text-2xl">🏁</span>
                    <p className="text-xs font-medium text-gray-700 mt-1">{isTamil ? 'போட்டி' : 'Race'}</p>
                  </Link>
                </div>
                
                <div className="text-center">
                  <Link
                    href={`/kural-playing?kural=${kuralOfDay.id}`}
                    className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500 to-red-500 text-white px-6 py-2.5 rounded-full font-semibold hover:shadow-lg hover:scale-105 transition-all"
                  >
                    <span>🎮</span>
                    <span>{isTamil ? 'இப்போது விளையாடு' : 'Play Now'}</span>
                    <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
